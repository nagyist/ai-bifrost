...
7. Set **ARN**: Required for aliases mapping
...
- `arn` is required for URL formation - `aliases` mapping is ignored without it.
- When using `arn` + `aliases`, Bifrost uses model profiles; otherwise forms path with incoming model name directly.
    ...
    "keys": [
        {
            ...
            "models": ["gemini-pro", "gemini-pro-vision", "fine-tuned-gemini-2.5-pro"],
            "weight": 1.0,
            "aliases": {
