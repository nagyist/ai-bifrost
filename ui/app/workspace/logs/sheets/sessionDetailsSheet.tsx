...
import { useLazyGetLogSessionByIdQuery } from "@/lib/store/apis/logsApi";
...
export function SessionDetailsSheet({
		...
		() => [
			{
				...
				helper: sessionLogs.length < totalCount ? `(${sessionLogs.length.toLocaleString()} loaded)` : undefined,
			},
			{
				label: "Total Cost",
			...
			{
				label: "Started",
				value: summary.startedAt ? moment(summary.startedAt).format("MMM D, YYYY hh:mm:ss A") : "N/A",
				size: "sm",
			},
			{
				label: "Latest Update",
				value: summary.latestAt ? moment(summary.latestAt).format("MMM D, YYYY hh:mm:ss A") : "N/A",
				size: "sm",
			},
			{
				label: "Duration",
				value: formatDurationFromMs(summary.durationMs),
			},
		],
		[summary, totalCount, sessionLogs.length],
	);
			...
			try {
				...
				if (result.data) {
					...
					setSummary({
						...
						latestAt: result.data.latest_at || "",
	...
	useEffect(() => {
		...
		setHasMore(false);
		setSummary({ totalCost: 0, totalTokens: 0, startedAt: "", latestAt: "", durationMs: 0 });
	...
	useEffect(() => {
		...
		const unsubscribe = subscribe("log", (data) => {
			...
			// Skip summary deltas for updates on logs we haven't loaded — the server
			// totals already include their values, so a zero-baseline delta would double-count.
			if (operation === "update" && oldLog === undefined) {
				return;
			}

			setSummary((prev) => {
				const costDelta = (log.cost || 0) - (oldLog?.cost || 0);
				const tokensDelta = (log.token_usage?.total_tokens || 0) - (oldLog?.token_usage?.total_tokens || 0);
				const latestAt = log.timestamp && (!prev.latestAt || new Date(log.timestamp) > new Date(prev.latestAt))
					? log.timestamp
					: prev.latestAt;
				const durationMs = prev.startedAt && latestAt
					? new Date(latestAt).getTime() - new Date(prev.startedAt).getTime()
					: prev.durationMs;
				return {
					...prev,
					totalCost: prev.totalCost + costDelta,
					totalTokens: prev.totalTokens + tokensDelta,
					latestAt,
					durationMs,
				};
			});
