...
export const modelProviderKeySchema = z
	...
	.refine(
		(data) => {
				// If bedrock_key_config, azure_key_config, vertex_key_config, vllm_key_config, or replicate_key_config is present, value is not required
				if (data.bedrock_key_config || data.azure_key_config || data.vertex_key_config || data.vllm_key_config || data.replicate_key_config) {
				return true;
			}
