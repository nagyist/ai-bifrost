...
// RealtimeWebRTCURL returns the HTTPS endpoint used for OpenAI's SDP exchange.
// GA endpoint: /v1/realtime/calls — model is in the multipart session payload, not the URL.
func (provider *OpenAIProvider) RealtimeWebRTCURL(_ string) string {
	base := provider.networkConfig.BaseURL
	return base + "/v1/realtime/calls"
}
...
// RealtimeWebRTCHeaders returns the headers required for the OpenAI Realtime WebRTC SDP exchange.
func (provider *OpenAIProvider) RealtimeWebRTCHeaders(key schemas.Key) map[string]string {
	headers := map[string]string{
		"Authorization": "Bearer " + key.Value.GetValue(),
	}
	for k, v := range provider.networkConfig.ExtraHeaders {
...
func (provider *OpenAIProvider) RealtimeTurnStartEvent() schemas.RealtimeEventType {
	return schemas.RTEventResponseCreate
}
...
func (provider *OpenAIProvider) ShouldForwardRealtimeEvent(event *schemas.BifrostRealtimeEvent) bool {
	return event == nil || event.Type != schemas.RTEventRateLimitsUpdated
}
...
func (provider *OpenAIProvider) CreateRealtimeClientSecret(
	...
	if resp.StatusCode() < fasthttp.StatusOK || resp.StatusCode() >= fasthttp.StatusMultipleChoices {
		return nil, ParseOpenAIError(resp, schemas.RealtimeRequest, provider.GetProviderKey(), requestedModel)
	}
	...
	out.ExtraFields.ModelRequested = requestedModel
...
func (provider *OpenAIProvider) ToProviderRealtimeEvent(bifrostEvent *schemas.BifrostRealtimeEvent) (json.RawMessage, error) {
	...
	if bifrostEvent.Session != nil {
		...
		mergeRealtimeExtraParams(sess, bifrostEvent.Session.ExtraParams)
		out["session"] = sess
	}
	...
	for _, output := range outputs {
		...
		for _, block := range output.Content {
			...
			case strings.TrimSpace(block.Refusal) != "":
				sb.WriteString(strings.TrimSpace(block.Refusal))
			}
		}
	}
	return sb.String()
