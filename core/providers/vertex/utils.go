...
func getRequestBodyForAnthropicResponses(ctx *schemas.BifrostContext, request *schemas.BifrostResponsesRequest, deployment string, providerName schemas.ModelProvider, isStreaming bool, isCountTokens bool, betaHeaderOverrides map[string]bool) ([]byte, *schemas.BifrostError) {
	...
	if ctxExtraHeaders, ok := ctx.Value(schemas.BifrostContextKeyExtraHeaders).(map[string][]string); ok {
		betaHeaders := anthropic.FilterBetaHeadersForProvider(ctxExtraHeaders[anthropic.AnthropicBetaHeader], schemas.Vertex, betaHeaderOverrides)
